...
class TestAiConnectionUseCase(
    ...
    suspend operator fun invoke(
        ...
        val sample =
            ListWordModel(
                word = "bahasa",
                listWords = listOf(WordModel("bahasa", listOf(MeaningModel("n", "sistem lambang bunyi")))),
            ).toWordStudyRequest("id") ?: return AppResult.Error(DataError.BadRequest)
        val resolvedApiKey =
            apiKey.trim().ifBlank {
        ...
        return when (val result = wordStudyRepository.generateWithCustomProvider(sample, credentials)) {
